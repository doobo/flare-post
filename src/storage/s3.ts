...
function parseS3Config(config: StorageConfig): { endpoint: string; bucket: string; region: string } {
  let endpoint: string;
  let bucket = '';
  let region = 'us-east-1';

  if (config.refreshToken) {
    region = config.refreshToken;
  }
  ...
  if (config.uploadUrl) {
    ...
    if (hostParts.length >= 4 && hostParts[1] === 's3') {
      bucket = hostParts[0];
      endpoint = `${url.protocol}//${hostParts.slice(1).join('.')}`;
    } else {
    ...
    throw new Error("Upload URL is required for S3 storage");
  }
...
export class S3Adapter implements IStorageAdapter {
  ...
  constructor(config: StorageConfig) {
    this.config = config;
    const parsed = parseS3Config(config);
  ...
  async upload(file: File | Blob, filename: string): Promise<UploadResult> {
    if (!this.config.accessKey || !this.config.secretKey) {
      throw new Error("Access Key and Secret Key are required for S3 storage");
    }
    ...
    await awsSign('PUT', url, headers, bodyHash, this.config.accessKey, this.config.secretKey, this.region, 's3');
  ...
  async delete(fileUrl: string): Promise<boolean> {
    ...
    await awsSign('DELETE', url, headers, bodyHash, this.config.accessKey, this.config.secretKey, this.region, 's3');
